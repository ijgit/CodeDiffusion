...
std::vector<int> calculateDivisors(int num) {
    ...
    for (int i = 1; i * i <= num; ++i) {
        if (num % i == 0) {
            ...
            if (i != num / i) {
                divisors.push_back(num / i); // Add num / i as a divisor
            }
        }
    }
    return divisors;
...
PopulationGenerationRule::ResultKind InitFillTileSize::Apply(SketchPolicyNode* policy, State* state,
  ...
  if (policy->target_states.defined() and policy->target_sketch != ""){
    // Randomly select a target_sp_step from policy's target_sp_steps
    auto target_state = policy->target_states[(*rand_gen)() % policy->target_states.size()];
    auto p_target_state = target_state.CopyOnWrite();
    ...
    for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
      if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
        ...
        std::vector<int> shared_lengths;

        for (size_t state_idx = 0; state_idx < pstate->transform_steps.size(); ++state_idx){
          if (auto target_state_ps = p_target_state->transform_steps[state_idx].as<SplitStepNode>()) {
            
            const int _stage_id = target_state_ps->stage_id;
            const int _iter_id = target_state_ps->iter_id;
            const int _extent = GetIntImm(target_state_ps->extent.value());
            std::vector<int> _lengths;
            for (size_t i = 0; i < target_state_ps->lengths.size(); ++i){
              _lengths.push_back(GetIntImm(target_state_ps->lengths[i].value()));
            }

            if (_stage_id == ps->stage_id && _iter_id == ps->iter_id) {
              shared_lengths.clear();

              if (_extent == extent){
                // 일치하는 경우
                for (const auto& len : _lengths) {
                  lengths.push_back(len);
                }
              }
              else {
                // 일치하지 않는 경우, 경우의 수가 세 가지 있음

                std::vector<std::pair<int, int>> len_idx_pair;
                std::vector<int> divisors_of_extent;

                for (size_t i = 0; i < _lengths.size(); i++) {
                  len_idx_pair.push_back(std::make_pair(_lengths[i], i));
                  shared_lengths.push_back(1);
                }
                std::sort(len_idx_pair.begin(), len_idx_pair.end(), std::greater<std::pair<int, int>>());
                ...
                if (random_choice == 0){
                  // 그대로 사용
                  int sp_step_len_mul = 1;
                  for (const auto& num : _lengths) {
                      sp_step_len_mul *= num;
                  }
                  if (extent % sp_step_len_mul == 0){
                    // 호환 가능하면 그대로 사용
                    for (const auto& len : _lengths) {
                      lengths.push_back(len);
                    }
                    ...
                    // 가장 가까운 약수로 변경
                    for (size_t i = 0; i < len_idx_pair.size(); i++){
                      if (len_idx_pair[i].first == 1){
                        shared_lengths[len_idx_pair[i].second] = 1;
                      }
                      else{
                        divisors_of_extent = calculateDivisors(_extent);
                        auto closest_divisor = std::min_element(divisors_of_extent.begin(), divisors_of_extent.end(), 
                          [&](int a, int b) { return std::abs(a - len_idx_pair[i].first) < std::abs(b - len_idx_pair[i].first); });

                        shared_lengths[len_idx_pair[i].second] = *closest_divisor;
                        _extent /= *closest_divisor;
                      }
                    }
                    for (size_t i = 0; i < shared_lengths.size(); i++){
                      lengths.push_back(shared_lengths[i]);
                    }
                  }
                }
                  ...
                  for (size_t i = 0; i < len_idx_pair.size(); i++){
                    if (len_idx_pair[i].first == 1){
                      ...
                      shared_len = float(extent) * (float(len_idx_pair[i].first) / float(_extent));
                      divisors_of_extent = calculateDivisors(_extent);
                      auto closest_divisor = std::min_element(divisors_of_extent.begin(), divisors_of_extent.end(), 
                        [&](int a, int b) { return std::abs(a - shared_len) < std::abs(b - shared_len); });
                      int reguralized_len = *closest_divisor;
                      shared_lengths[len_idx_pair[i].second] = reguralized_len;
                      _extent /= reguralized_len;
                    }
                  }
                  for (size_t i = 0; i < shared_lengths.size(); i++){
                    lengths.push_back(shared_lengths[i]);
                  ...
                  for (const auto& len : candidate_lengths) {
                    lengths.push_back(len);
                  }
                }
              }
        ...
        pstate->transform_steps.Set(
            ...
            SplitStep(ps->stage_id, ps->iter_id, ps->extent,
                      Array<Optional<Integer>>(lengths.begin(), lengths.end()),
                      ps->inner_to_outer));
      }
    }
  }
  else {
  // if (! policy->target_states.defined()){
    for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
      if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
        bool all_defined = true;
        for (const auto& len : ps->lengths) {
...
/*
    ...
    const auto& target_sp_step = policy->target_sp_steps[(*rand_gen)() % policy->target_sp_steps.size()];
    ...
    for (size_t step_id = 0; step_id < (*state)->transform_steps.size(); ++step_id) {
      if (auto ps = (*state)->transform_steps[step_id].as<SplitStepNode>()) {
        ...
        for (const auto& sp_step : target_sp_step) {
          const int _stage_id = std::get<0>(sp_step);
          const int _iter_id = std::get<1>(sp_step);
          const int _extent = std::get<2>(sp_step);
          std::vector<int> _lengths = std::get<3>(sp_step);
...
PopulationGenerationRule::ResultKind InitUnroll::Apply(SketchPolicyNode* policy, State* state,
  ...
  for (size_t stage_id = 0; stage_id < (*state)->stages.size(); ++stage_id) {
    ...
    if (HasReduceIter(stage)) {
      ...
      if (policy->target_states.defined() and policy->target_sketch != ""){
        std::vector<size_t> random_prob;
        random_prob.push_back(0); random_prob.push_back(1);

        auto target_state = policy->target_states[(*rand_gen)() % policy->target_states.size()];
        auto p_target_state = target_state.CopyOnWrite();

        for (size_t i = 0; i < p_target_state->transform_steps.size(); i++){
          if (auto ps = p_target_state->transform_steps[i].as<PragmaStepNode>()){
            if (ps->stage_id == (int)stage_id){
              state->pragma(stage_id, (*state)->stages[stage_id]->iters[0], ps->pragma_type);
            }
            else {
              continue;
            }
