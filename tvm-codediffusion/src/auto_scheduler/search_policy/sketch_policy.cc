...
SketchPolicy::SketchPolicy(SearchTask task, CostModel program_cost_model,
                           ...
                           Array<State> target_states, std::string target_sketch) {
  ...
  // std::cout << "target sketch: " << node->target_sketch << std::endl;

  if (node->target_states.defined()) {
    for (size_t state_idx = 0; state_idx < node->target_states.size(); ++state_idx){
      auto state = node->target_states[state_idx];
      auto pstate = state.CopyOnWrite();

      std::vector<std::tuple<size_t, size_t, size_t, std::vector<int>>> sp_steps;

      for (size_t step_id = 0; step_id < pstate->transform_steps.size(); ++step_id) {
        if (auto ps = pstate->transform_steps[step_id].as<SplitStepNode>()) {
          std::vector<int> lengths;
          for (size_t i = 0; i < ps->lengths.size(); ++i){
            lengths.push_back(GetIntImm(ps->lengths[i].value()));
          }
          sp_steps.push_back(std::make_tuple(
            ps->stage_id, 
            ps->iter_id, 
            GetIntImm(ps->extent.value()), 
            lengths
          ));
        }

        // if (auto ps = pstate->transform_steps[step_id].as<PragmaStepNode>()){
// 
        // }

      // std::cout << "auto unroll max step: " << std::endl;
      // for (size_t i = 0; i < (*state)->transform_steps.size(); i++){
      //   if (auto ps = (*state)->transform_steps[i].as<PragmaStepNode>()){
      //     std::cout << ps->iter_id << ", " << ps->pragma_type << std::endl;
      //   }
      // }
      // std::cout << std::endl;

      }
      node->target_sp_steps.push_back(sp_steps);
    }
    // for (size_t step_id=0; step_id < node->target_states->transform_steps.size(); ++step_id){
    //   if (auto ps = node->target_states->transform_steps[step_id].as<SplitStepNode>()) {

    //     std::vector<int> lengths;
    //     for (size_t i = 0; i < ps->lengths.size(); ++i){
    //       lengths.push_back(GetIntImm(ps->lengths[i].value()));
    //     }
    //     node->target_sp_steps.push_back(std::make_tuple(
    //       ps->stage_id, 
    //       ps->iter_id, 
    //       GetIntImm(ps->extent.value()), 
    //       lengths
...
std::pair<Array<MeasureInput>, Array<MeasureResult>> SketchPolicyNode::ContinueSearchOneRound(
  ...
  // std::cout << "[SketchPolicy::SketchPolicy] target_state:" << target_state << std::endl;
  ...
  //   std::cout << "best_states sketch_id: " <<  *(int *) &best_states[i]->sketch_id << std::endl;    
  // }
...
Array<State> SketchPolicyNode::SearchOneRound(int num_random_states, Array<State>* random_states) {
  ...
  if (sketch_cache_.empty()) {
    ...
    if (target_sketch != ""){
      Array<State> tmp_sketch_cache_;
      for (size_t i = 0; i < sketch_cache_.size(); i++){
        if (sketch_cache_[i].ToStr() == target_sketch){
          tmp_sketch_cache_.push_back(sketch_cache_[i]);
        }
      }
      if (tmp_sketch_cache_.empty()){
        std::cout << "[LOG] target sketch not found" << std::endl;
        target_sketch = "";
      }
      else {
        // std::cout << "[DEBUG] rm target sketch: " << tmp_sketch_cache_.size() << std::endl;
        sketch_cache_ = tmp_sketch_cache_;
      }
    }
    for (size_t i = 0; i < sketch_cache_.size(); i++){
      *(int *)&sketch_cache_[i]->sketch_id = i;
    }
    for (State state : sketch_cache_){
      std::cout << "sketch_cache sketch_id: " <<  state->sketch_id << ", " << *(int *) &state->sketch_id << std::endl;
    }

    double duration = std::chrono::duration_cast<std::chrono::duration<double>>(
                        std::chrono::high_resolution_clock::now() - tic_begin)
                        .count();
    std::cout << "[LOG] Time elapsed for generating sketches:" << duration << std::endl;

    // for (size_t i = 0; i < sketch_cache_.size(); i++){
    //   for (size_t j = 0; j < sketch_cache_[i]->transform_steps.size(); j++){
    //     std::cout << sketch_cache_[i]->transform_steps[j]->GetTypeKey() << ", ";
    //   }
    //   std::cout << std::endl;
    // }

    // std::cout << "[LOG] Generated Sketches" << std::endl;
    // for (size_t i = 0; i < sketch_cache_.size(); ++i) {
    //   auto state = sketch_cache_[i];

    //   std::cout << state.ToStr() << std::endl;
    //   std::cout << "transform_steps: ";
    //   for (size_t step_id = 0; step_id < state->transform_steps.size(); ++step_id){
    //     std::cout << state->transform_steps[step_id]->GetTypeKey() << ", ";
    //   }
    //   std::cout << std::endl;
    // }
    // std::cout << "========================================" << std::endl;
  ...
  //   std::cout << "init_population sketch id: " <<  *(int *) &state->sketch_id << std::endl;
  // }

  double duration = std::chrono::duration_cast<std::chrono::duration<double>>(
                        std::chrono::high_resolution_clock::now() - tic_begin)
                        .count();
  std::cout << "[LOG] Time elapsed for sampling initial population:" << duration << std::endl;                  
  ...
  for (int i = 0; i < num_use_measured; i++) {
    ...
    // init_population_sketch_ids.push_back(measured_state_sketch_ids[indices[i]]);
  }
  ...
  auto tic_begin_evolutionary_search = std::chrono::high_resolution_clock::now();
  Array<State> best_states = EvolutionarySearch(init_population, num_measure_per_iter_*2);
  double duration_evolutionary_search = std::chrono::duration_cast<std::chrono::duration<double>>(
                        std::chrono::high_resolution_clock::now() - tic_begin_evolutionary_search)
                        .count();
  std::cout << "[LOG] Time elapsed for evolutionary search:" << duration_evolutionary_search << std::endl;                        
...
Array<State> SketchPolicyNode::GenerateSketches() {
  ...
  int max_innermost_split_factor = GetIntParam(params, SketchParamKey::max_innermost_split_factor);
  std::cout << "max_innermost_split_factor:" << max_innermost_split_factor << std::endl;

  for (size_t i = 0; i < out_states.size(); i++){
    auto state = out_states[i];
    auto pstate = state.CopyOnWrite();
    for (size_t step_id = 0; step_id < pstate->transform_steps.size(); ++step_id) {
      if (pstate->transform_steps[step_id]->IsInstance<SplitStepNode>()) {
        if (auto ps = pstate->transform_steps[step_id].as<SplitStepNode>()) {
          std::cout << "[LOG] SplitStep Info:" << ps->extent << ", " << ps->lengths << ", " << ps->inner_to_outer << std::endl;
        }
      }
    }
  }
    
  // for (auto state: out_states){
  //   std::cout << "out_state sketch_id: " << state.sketch_id << ", " << state->sketch_id << std::endl;
  // }
...
Array<State> SketchPolicyNode::SampleInitPopulation(const Array<State>& sketches) {
  ...
  while (static_cast<int>(out_states.size()) < sample_init_min_pop_) {
    ...
    // std::vector<size_t> tmp_sketch_ids(population);
    support::parallel_for(0, population, [this, &temp_states, &sketches, &rand_gens](int index) {
      // Randomly choose a sketch
      size_t sketch_id = (rand_gens[index])() % sketches.size();
      State tmp_s = sketches[sketch_id];

      // std::cout << "tmp_s sketch id: " << tmp_s.sketch_id << ", " << tmp_s.sketch_id << std::endl;
      // std::cout << "sketches[sketch_id] sketch id: " << sketches[sketch_id].sketch_id << ", " << sketches[sketch_id].sketch_id << std::endl;
      ...
      if (valid) {
        ...
        temp_states[index].sketch_id = sketch_id;
        // std::cout << "temp_states sketch id: " << temp_states[index].sketch_id << ", " << temp_states[index]->sketch_id << std::endl;
      }
    });

    // Filter out the states that were failed to apply initial rules
    Array<State> cand_states;
    // std::vector<size_t> cand_sketch_ids;
    for (State tmp_s : temp_states) {
      if (tmp_s.defined()) {
        cand_states.push_back(std::move(tmp_s));
        *(int *) &cand_states.back()->sketch_id = tmp_s.sketch_id;
        // std::cout << "[SampleInitPopulation] cand_states sketch id: " <<  *(int *) &cand_states.back()->sketch_id << std::endl;
      } else {
        fail_ct++;
      }
    }
    ...
    if (!cand_states.empty()) {
      ...
      for (size_t i = 0; i < cand_states.size(); i++) {
        ...
        if (pop_scores[i] > -1e10 && explored_state_strs.count(state_str) == 0) {
          ...
          *(int *) &out_states.back()->sketch_id = *(int *) &cand_states[i]->sketch_id;
          // std::cout << "out_sstates sketch id: " <<  *(int *) &out_states.back()->sketch_id << std::endl;
  ...
  // for (size_t i = 0; i < out_states.size(); ++i) {
  //   auto state = out_states[i];
  //   std::cout << state.ToStr(true) << std::endl;
  //   std::cout << state.ToStr(false) << std::endl;
  //   std::cout << "transform_steps: ";
  //   for (size_t step_id = 0; step_id < state->transform_steps.size(); ++step_id){
  //     std::cout << state->transform_steps[step_id]->GetTypeKey() << ", ";
  //     if (state->transform_steps[step_id]->IsInstance<SplitStepNode>()) {
  //       auto step = state->transform_steps[step_id].as<SplitStepNode>();
  //       std::cout << "(" << step->lengths << "), ";
  //     }
  //   }
  //   std::cout << std::endl;
  // }
...
Array<State> SketchPolicyNode::EvolutionarySearch(const Array<State>& init_population,
  ...
  // for (auto state : init_population){
  //   std::cout << "evolutionary search, init_population sketch id: " <<  *(int *) &state->sketch_id << std::endl;
  // }
    ...
    for (size_t i = 0; i < pnow->size(); ++i) {
      ...
      if (in_heap.count(state_str) == 0) {
        if (static_cast<int>(heap.size()) < out_size) {
          ...
          heap.back().first.sketch_id = (*pnow)[i]->sketch_id;
  ...
  // for (size_t i = 0; i < best_states.size(); ++i) {
    // std::cout << "[EvolutionarySearch]best_states sketch_id: " <<  *(int *) &best_states[i]->sketch_id << std::endl;                   
  // }
  return best_states;
}
...
Array<MeasureInput> SketchPolicyNode::PickStatesWithEpsGreedy(const Array<State>& best_states,
  ...
  while (static_cast<int>(inputs.size()) < std::min(num_measure_per_iter_, remaining_n_trials)) {
    ...
    if (!measured_states_set_.count(state_str)) {
      // state.sketch_id = *(int *) &state->sketch_id;
      measured_states_set_.insert(std::move(state_str));
      measured_states_vector_.push_back(state);
      // int sketch_id = *(int *) &state->sketch_id;
      // State _sketch = sketch_cache_[sketch_id];
      std::string _sketch_str;
      if (*(int *) &state->sketch_id >= 0){
        _sketch_str = sketch_cache_[*(int *) &state->sketch_id].ToStr();
        // std::cout << "[MeasureInput] sketch str: " << _sketch_str << std::endl;
        // std::cout << "[MeasureInput] sketch_id: " << *(int *) &state->sketch_id << ", " << _sketch_str << std::endl;
        // inputs.push_back(MeasureInput(search_task, state, _sketch_str));
      }
      else{
        _sketch_str = "";
        // std::cout << "[MeasureInput] sketch_id: " << *(int *) &state->sketch_id << ", " << _sketch_str << std::endl;
  ...
  //   std::cout << "inputs sketch_id: " << inputs[i]->sketch_id << std::endl;    
  // }
...
TVM_REGISTER_GLOBAL("auto_scheduler.SketchPolicy")
    ...
    .set_body_typed([](SearchTask task, CostModel program_cost_model, Map<String, ObjectRef> params,
                       ...
                       std::string target_sketch) {
      return SketchPolicy(task, program_cost_model, params, seed, verbose, init_search_callbacks, target_states, target_sketch);
    });
