...
class TaskScheduler:
    ...
    def __init__(
        ...
        # for task in tasks:
        #     print(task.desc)
        #     if task_desc_to_group_name(task.desc) == TEST_GROUP_NAME:
        #         self.tasks.append(task)
        
        # for debuging (feature, latency)
        # self.tuning_log = {
        #     i: {
        #         'features': [],
        #         'latencies': [],
        ...
        self.features = []
        self.latencies = []
        self.tuning_log_path = log_file_name.replace(".tsv", "-log.pkl")
        self.result_path = log_file_name.replace(".tsv", "-result.pkl")
    ...
    def _tune_task(self, task_idx):
        ...
        for input, result in zip(measure_inputs, measure_results):
            
            # feature, normalized_throughput, task_id = get_per_store_features_from_measure_pairs([input], [result])
            cost = array_mean(result.costs)
            
            # _feature = feature.reshape(-1)
            # self.tuning_log[task_idx]['features'].append(feature.reshape(-1))
            # self.tuning_log[task_idx]['latencies'].append(normalized_throughput)
            # self.tuning_log[task_idx]['results'].append(cost)
            # self.tuning_log[task_idx]['task_ids'].append(task_id)
            # self.tuning_log[task_idx]['states'].append(input.state)
