...
(
    ...
    CUDA_VISIBLE_DEVICES=$gpu_num python $ansor_run --target=$target --model=$model\
        --log_dir=$log_dir\
...
(
    ...
    export PYTHONPATH=$TVM_HOME/python:${PYTHONPATH} 
    CUDA_VISIBLE_DEVICES=$gpu_num python $codediffusion_run --target=$target --model=$model\
        --log_dir=$log_dir\
        --group_type=sketch\
